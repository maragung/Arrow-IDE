...
public class TermSession {
    ...
    // ARROW-IDE MODIFICATION (see THIRD_PARTY_NOTICES.md): the message handler
    // is now created through a helper that tolerates construction on a thread
    // without a Looper (and the stubbed android.os classes of pure-JVM unit
    // tests). On a normal Android main thread the behavior is unchanged; when
    // no handler can be created the reader/writer threads are not started and
    // the emulator is driven directly instead.
    private Handler mMsgHandler = createMsgHandler();
    ...
    public TermSession(final boolean exitOnEOF) {
        ...
        mReaderThread = new Thread() {
            ...
            public void run() {
                ...
                // ARROW-IDE MODIFICATION (see THIRD_PARTY_NOTICES.md): guard
                // against a null message handler.
                if (exitOnEOF && mMsgHandler != null) {
    ...
    public void initializeEmulator(int columns, int rows) {
        ...
        mIsRunning = true;
        // ARROW-IDE MODIFICATION (see THIRD_PARTY_NOTICES.md): only start the
        // I/O threads when the corresponding stream has been connected. This
        // keeps TermSession constructible (and the emulator drivable) in
        // pure-JVM unit tests, where the writer thread's Looper is unavailable.
        if (mTermIn != null) {
            mReaderThread.start();
        }
        if (mTermOut != null) {
