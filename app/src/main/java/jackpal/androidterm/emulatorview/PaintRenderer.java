...
class PaintRenderer extends BaseTextRenderer {
    public PaintRenderer(int fontSize, ColorScheme scheme) {
        ...
        // ARROW-IDE MODIFICATION (see THIRD_PARTY_NOTICES.md): FloatMath was
        // replaced with Math (android.util.FloatMath is deprecated).
        mCharHeight = (int) Math.ceil((double) mTextPaint.getFontSpacing());
        mCharAscent = (int) Math.ceil((double) mTextPaint.ascent());
